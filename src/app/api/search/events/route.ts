...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const data = await searchEvents(nq, pag);
    logUnifiedSearchRequest("events", nq, pag, { resultCount: data.items.length, totalEstimate: data.totalEstimate });
    return NextResponse.json({ ok: true, data });
  } catch (e) {
    console.error("[GET /api/search/events]", e);
